      ...
      <v-flex xs2 v-if="isFinancialOfficer || isTreasury" class="pb-3 pt-4 grey-background" v-for="(item, i) in tokenStat" :key="i + '-stat'"> 
        <v-card elevation="0" height="200px" class="grey-background">
          <v-list dense class="pa-1 pb-2 grey-background" :class="[{ 'delimiter': (i + 1) != tokenStat.length }]">
            ...
            <v-list-tile>
              <v-list-tile-content class="headline bold" :class="item.isGreen ? 'green--text text--darken-1': 'primary--text text--darken-1'">{{item.amount | currency}}</v-list-tile-content>
            </v-list-tile>
            <v-list-tile>
                      ...
                      <template row>
                        ...
                        <v-data-table
                          ...
                          <template slot="footer">
                            <td v-for="(header, i) in awardHeaders" class="body-2 bold">

  ...
  <v-card class="pa-4 full-height">
    ...
    <div class="pt-4" v-if="!isPublished && isResearchGroupMember">
      ...
      <div v-if="isProposed" class="c-mt-3 c-mb-3">
        <div class="subheading orange--text text-align-center">
          ...
          >proposed</router-link> as research content and locked for editing
        </div>
      </div>
    ...
    <div class="py-4">
      ...
      <div v-if="isPublished">
        <v-layout row v-for="(author, index) in contentAuthorsList" :key="`author-${index}`" :class="{'pb-1' : index == 0, 'py-1': index != 0}">
          <platform-avatar
            :user="author"
            :size="40"
            link-to-profile
      ...
      <div v-else>
        <v-layout row justify-space-between align-baseline v-for="(member, index) in draftAuthorsList" :key="`author-${index}`" :class="{'pb-1' : index == 0, 'py-1': index != 0}">
          <div>
            <platform-avatar
              :user="member"
              :size="40"

  ...
  <base-page-layout content-class="xl9 lg9 md9 sm12 xs12" right-sidebar-class="xl3 lg3 md3 sm12 xs12">
    <div slot="content" class="full-height">
      <v-card class="full-height">
              ...
              <div id="reviews">
                ...
                <div class="py-2">
                  <div v-for="(review, i) in contentReviewsList" :key="`review-${i}`">
                    <review-tile class="my-2" :review="review"
                                 :researchContentType="content.content_type"></review-tile>
              ...
              <v-card class="py-4 px-5">
                <v-layout id="reviews" class="py-2" row>
                  ...
                  <v-flex grow align-self-center pl-5>
                    <div class="pb-3">
                      <div v-if="!contentReviewsList.length" class="pb-1 subheading half-bold">There are no reviews for
                        this material yet
                      </div>
                      <div v-if="userHasResearchExpertise && !userHasReview">You will get <span class="body-2">approximately 3000 ECI reward in {{userRelatedExpertise.map(exp => exp.discipline_name).join(', ')}}</span>
                        for your contribution to this project
                      </div>
                      <div v-else-if="userHasResearchExpertise && userHasReview" class="pb-1 subheading half-bold">You
                        have reviewed this material already
                      </div>
                      <div v-else-if="!userHasResearchExpertise">Users with expertise in <span class="body-2">{{research.disciplines.map(d => d.name).join(', ')}}</span>
        ...
        <v-dialog v-if="research" v-model="proposeContent.isOpen" persistent transition="scale-transition"
                  max-width="600px">
          <v-card class="pa-4">
            <v-card-title>

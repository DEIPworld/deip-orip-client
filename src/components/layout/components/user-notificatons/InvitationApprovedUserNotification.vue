<template>
  <v-card class="pa-3 clickable user-notification" @click="clickNotification(notification)">
    <v-layout column align-space-between>
      <div>
        <span class="primary--text half-bold">{{ inviteeProfile | fullname }}</span> approved invitation and now is a member of "<span class="primary--text half-bold">{{notification.metadata.researchGroup.name}}</span>"
      </div>
...
export default {
  name: "InvitationApprovedUserNotification",
  props: {

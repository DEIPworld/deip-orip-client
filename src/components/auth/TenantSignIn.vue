      ...
      login() {
        if (this.$refs.form.validate()) {
          ...
          authService.signIn({
            ...
            .then((response) => {
              if (response.success) {
                const decoded = accessService.decodedToken(response.jwtToken);
                ...
                if (isValid) {
                  ...
                  accessService.setAccessToken(response.jwtToken, this.privKey)
                  this.isChecking = false;
                  ...
                  accessService.clearAccessToken();
                  this.isChecking = false;
                  this.$store.dispatch('layout/setError', {message: `Invalid private key for "${this.username}"`});
                }

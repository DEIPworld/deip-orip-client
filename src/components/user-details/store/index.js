...
const actions = {
    ...
    loadUser({ dispatch, commit }, { username }) {
        ...
        const invitesLoad = stateApiSvc.loadUserInvites(username);
        const groupsLoad = stateApiSvc.loadGroups(username);
        ...
        return Promise.all([accountLoad, profileLoad, expertiseLoad, invitesLoad, groupsLoad])
            .then(result => {
                ...
                return stateApiSvc.loadResearchList(username, groups.map(group => group.id))
            })
            .then(researchList => {
                const [account, profile, expertise, invites, groups] = tmp;

                commit('SET_USER_ACCOUNT', account);
                commit('SET_USER_PROFILE', profile);
                commit('SET_RESEARCH_GROUPS', groups);
                commit('SET_EXPERTISE', expertise);
    ...
    loadUserAccount({ commit }, { username }) {
        commit('SET_USER_ACCOUNT_LOADING_STATE', true)

        return stateApiSvc.loadUserAccount(username)
            .then(account => {
                commit('SET_USER_ACCOUNT', account);
            })
            .finally(() => {
    ...
    loadGroups({ commit }, { username }) {
        return stateApiSvc.loadGroups(username)
            .then(groups => {
                commit('SET_RESEARCH_GROUPS', groups);
            })
            .finally(() => {
    ...
    loadResearchList({ commit, state }, { username, groupIds }) {
        commit('SET_USER_RESEARCH_LOADING_STATE', true)

        // researhes where user took participation 
        return stateApiSvc.loadResearchList(username, groupIds)
            .then(data => {
    ...
    loadExpertise({ commit }, { username }) {
        commit('SET_USER_EXPERTISE_LOADING_STATE', true)

        return stateApiSvc.loadExpertise(username)
            .then(data => {
    ...
    loadUserProfile({ commit },  { username, loading }) {
        if (loading) commit('SET_USER_PROFILE_LOADING_STATE', true)

        return stateApiSvc.loadUserProfile(username)
            .then(profile => { 
                commit('SET_USER_PROFILE', profile);
            }, (err) => {
                console.log(err)
    ...
    loadUserInvites({ commit }, { username }) {
        ...
        return stateApiSvc.loadUserInvites(username)
            .then(invites => {
                commit('SET_USER_INVITES', invites);
            }).finally(() => {

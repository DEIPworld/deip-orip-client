    ...
    <div>
      ...
      <div class="user-profile-info-container spinner-container">
        ...
        <v-layout row wrap v-if="isLoadingUserAccount === false && isLoadingUserProfile === false">
          ...
          <v-flex xs12 sm8>
            <div class="display-1 font-weight-medium pt-3">{{userInfo | fullname}} <span
              v-if="(userInfo.profile && userInfo.profile.firstName)" class="caption username-caption grey--text">({{userInfo.account.name}})</span>
              <v-btn
                v-if="isOwner"

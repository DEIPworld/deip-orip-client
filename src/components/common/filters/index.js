...
Vue.filter('avatarSrc', function (avatar, width, height, noCache, isRound) {
    let url =  `${window.env.DEIP_SERVER_URL}/public/files/avatars/${avatar}?width=${width}&height=${height}`
    if (noCache) {
        url = `${url}&noCache=${noCache}`
    }
    if (isRound) {
        url = `${url}&round=${isRound}`
    }
    return url;
});

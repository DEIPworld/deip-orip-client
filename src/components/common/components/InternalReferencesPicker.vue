    ...
    <v-layout column>
      <div class="full-width">
        ...
        <div v-if="showSelected">
          ...
          <v-layout row justify-space-between align-center :class="{'py-2': i > 0}" v-for="(ref, i) in selected"
                    :key="i + '-picked'">
            <div class="body-1 pr-4"><span>{{ref.title}} ({{ref.research_title}})</span></div>
            <div class="text-xs-right">
              <v-btn @click="removeReference(ref)" small outline class="ma-0">Remove</v-btn>
            </div>
          </v-layout>
        </div>
        ...
        <v-layout row justify-space-between align-center :class="{'py-2': i > 0, 'pt-4': i == 0}"
                  v-for="(ref, i) in list" :key="i + '-selectable'">
          <div class="pr-4">
          ...
          <div class="text-xs-right">
            <v-btn @click="addReference(ref)" outline small color="primary" class="ma-0">+ Add</v-btn>
          </div>
        </v-layout>
      ...
      searchReferences: _.debounce(function f() {
        ...
        function filter() {
          return this.references.filter((content) => {
            return content.title.toLowerCase().startsWith(term)

<template>
  <div>
    <v-layout v-for="(criteria, i) in criterias" :key="`review-criteria-${i}`" row justify-space-between align-center
...
<script>
  ...
  export default {
    ...
    computed: {
      criterias() {
        return researchContentReviewsService.getAssessmentCriteria(this.researchContentType)
      }
    },

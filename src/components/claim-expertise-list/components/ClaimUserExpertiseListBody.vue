  ...
  <div>
    ...
    >
      <template v-slot:item="{item}">
        <tr>
          <td class="">
            <router-link class="a" :to="{ name: 'UserDetails', params: { account_name: item.username }}">
              {{ item.user | fullname }}
            </router-link>
          </td>
          ...
          <td>
            <div>
              ...
              <span v-if="item.status == 'approved'" class="caption grey--text">(approved)</span>
            </div>
          </td>

          <td class="text--center">
            {{ item.created_at | dateFormat('HH:mm D MMM YYYY', true) }}
          </td>
          <td class="text--center">
            {{ item.publications.length }}
          </td>

          <td class="legacy-row legacy-justify-end">
            <v-btn
              color="primary"
              text
              :to="{
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      pagination: {
        get() {
          return { ...this.sorting, rowsPerPage: -1 };
        },

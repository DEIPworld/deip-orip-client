      ...
      <v-flex xs9>
        ...
        <v-card class="pa-4 full-height">
          <v-layout row wrap>
            ...
            <v-flex xs12>
              <div class="headline pt-4">Award Receivers</div>
              <div v-for="(funding, fundingIdx) in fundings" :key="`${fundingIdx}-funding`">
                ...
                <v-layout row>
                  ...
                  <v-flex xs3 v-if="fundingIdx != 0">
                    ...
                    <div class="subheading font-weight-bold">PI</div>
                    <v-select 
                      class="pa-0 ma-0"
                ...
                <v-layout row>
                  <v-flex xs8>
                    <div v-if="fundingIdx == 0">
                      ...
                      <v-text-field :rules="[rules.allowedOverhead]" v-model="funding.overhead" suffix="%" mask="##"></v-text-field>
                    </div>
                  </v-flex>
                  <v-flex xs1></v-flex>
                  <v-flex xs3>
              ...
              <v-layout justify-space-between row py-3>
                <div>
                ...
                <div>
                  <v-btn class="ma-0" block color="primary" @click="save()" :disabled="saveIsDisabled || isSaving" :loading="isSaving">
                    Save
                  </v-btn>
                </div>
              </v-layout>
    ...
    export default {
            ...
            return {
              ...
              rules: {
                ...
                allowedOverhead: (v) => {
                  ...
                  } else if (val > this.MAX_UNIVERSITY_OVERHEAD){
        ...
        methods: {
          ...
          setAwardSourcesList(funding) {
            ...
            let foundAwardSources = this.fundings
              ...
              .map((f => {
                ...
                return { name, user: f.researcher.user };
              }));
          ...
          save() {
            ...
            let granAssetSymbol = blockchainService.getAssetSymbol(this.program.amount);
            let grantAssetPrecision = blockchainService.getAssetPrecision(this.program.amount);
            
            this.isSaving = true;

            for (let i = 1; i < this.fundings.length; i++) {
              let funding = this.fundings[i];
              let source = funding.awardSource.user.account.name;

...
const actions = {
      ...
      .then(() => {
        const applicationRefLoad = new Promise((resolve, reject) => {
          dispatch('loadResearchApplicationRef',
            {
              agency: state.program.agency_name,
              foaId: state.program.id,
              hash: state.application.packageHash,
              notify: resolve
            });
        });
  ...
  loadResearchApplicationRef({ state, commit, dispatch }, {
    agency, foaId, hash, notify
  }) {
    return grantsService.getApplicationPackageRef(agency, foaId, hash)
      .then((applicationRef) => {
    ...
    deipRpc.api.getGrantApplicationReviewsByGrantApplicationAsync(application_id)
      .then((items) => {
        ...
        return Promise.all([
          ...
          Promise.all(reviews.map((review) => deipRpc.api.getGrantApplicationAsync(review.grant_application_id)
  ...
  loadThirdpatyReviews({ state, dispatch, commit }, {
    currentApplication, group_permlink, research_permlink, notify
  }) {
    ...
    deipRpc.api.getResearchByAbsolutePermlinkAsync(group_permlink, research_permlink)
      .then((research) => deipRpc.api.getGrantApplicationsByResearchIdAsync(research.id))
      .then((applications) => {

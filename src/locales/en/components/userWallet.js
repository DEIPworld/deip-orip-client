export default {
  balance: 'Currencies',
  portfolio: 'Shares',
  groups: 'Groups',
  transfer: 'Transfer',
  deposit: 'Deposit',
  withdraw: 'Withdraw',
  ownershipShare: 'Your ownership share',
  pricePerToken: 'Price per token',
  ownershipValue: 'Your ownership value',
  totalValue: 'Total value',
  priceChange: 'Price change',
  tokenHolders: '# of token holders',
  sharePrice: 'Share price',
  shareHolders: 'Share holders',
  transferShare: 'Transfer share',
  cancel: 'Cancel',
  depositDialog: {
    depositFunds: 'Deposit funds',
    amountField: {
      label: 'Amount',
      placeholder: 'Amount'
    },
    confirmQualifiedField: 'I confirm that I am qualified investor'

  },
  withdrawDialog: {
    withdrawFunds: 'Withdraw funds',
    nameField: 'Full name (Beneficiary)',
    idanField: {
      label: 'IBAN',
      tooltip: 'International Bank Account Number. From 16 to 34 alphanumeric characters'
    },
    referenceField: {
      label: 'Reference number',
      tooltip: 'Most reference numbers will be found at the top of the application submission'
    },
    confirmQualifiedField: 'I confirm that I am qualified investor',
    beneficiaryField: 'Message to Beneficiary',
    amountField: 'Amount'
  },
  sendResearchTokensDialog: {
    transfer: 'Transfer Research Share',
    receiverField: 'Receiver',
    shareField: 'Share',
    submitBtn: 'Transfer'
  },
  sendTokensDialog: {
    transfer: 'Transfer',
    toField: 'To',
    amount: 'Amount',
    memoField: 'Memo - optional',
    submitBtn: 'Transfer'
  },
  translations: {
    name: {
      label: 'Name',
      placeholder: 'Full name'
    },
    card: {
      label: 'Card Number',
      placeholder: 'Card Number'
    },
    expiration: {
      label: 'Expiration'
    },
    security: {
      label: 'Security Code',
      placeholder: 'Code'
    }
  }
};
